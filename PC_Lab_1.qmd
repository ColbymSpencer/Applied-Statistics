...
1.1.2. Calculate the following values and store them as the objects x1, x2 en x3:
• ln(12)
• log 6 (216) 
• log(1000)
```{r}
...
1.1.3. Can you predict what the output is of these lines of code without running it? You can find that information on the help page
• log(0, base = 5) 
• log(-1)
```{r}
print(log(0,base = 5))
print(log(-1))
```
log of zero for any base is undefined. In r it returns -inf
The log function is only defined in 
...
### Exercise 2
Start by downloading and unzipping the file AS PClab1.zip from UFORA. Once you’ve done that, do the following exercises.
2.1.1. Open the project AS PClab1 by clicking on the file AS PClab1.Rproj. 
```{r}
...
3.2.2. Take a look at the structure of the object (see cheat sheet D for the relevant function).Can you interprete the output?
...
3.3.2. Compare the object employees with the object mydata from the example above by looking at its structure. Are both objects identical? If not, what difference do you see? 
```{r}
...
#### EXERCISES 3.4
For these exercises, take a look at the cheat sheets E and F.
3.4.1. Select the variabele location from allbirds and store this as the object loc. 
```{r}
...
### Exercise 4
EXERCISES 4.1
For these exercises, check cheat sheets F, G H. 4.1.1. Calculate the mean length of the birds in the data frame birds2000. 
